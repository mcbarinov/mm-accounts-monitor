...
class NameService(AppService):
    ...
    async def check_next_naming(self, naming: Naming) -> None:
        if not self.dynamic_values.check_namings:
            return
        ...
        need_to_check = await self.db.account_name.find(
            {"naming": naming, "checked_at": None}, limit=self.dynamic_configs.limit_naming_workers
        )
        if len(need_to_check) < self.dynamic_configs.limit_naming_workers:
            need_to_check += await self.db.account_name.find(
                {"naming": naming, "checked_at": {"$lt": utc_delta(minutes=-1 * self.dynamic_configs.check_name_interval)}},
                "checked_at",
                limit=self.dynamic_configs.limit_naming_workers - len(need_to_check),
            )
        if not need_to_check:
            return

        runner = AsyncTaskRunner(self.dynamic_configs.limit_naming_workers, name="check_names")
    ...
    async def check_account_name(self, id: ObjectId) -> Result[str | None]:
        ...
        match account_name.naming:
            case Naming.ENS:
                urls = self.network_service.get_rpc_urls(account_name.network)
                if not urls:
                    return Result.err("no_rpc_urls")
                res = await evm.get_ens_name(urls, account_name.account, proxies=self.dynamic_values.proxies)
            case Naming.ANS:
                res = await aptos.get_ans_name(account_name.account, proxies=self.dynamic_values.proxies)
            case Naming.STARKNET_ID:
                res = await starknet.get_starknet_id(account_name.account, proxies=self.dynamic_values.proxies)
            case _:
